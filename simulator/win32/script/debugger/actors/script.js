    ...
    if (column != null) {
      if (this._breakpoints[url]) {
        if (this._breakpoints[url][line]) {
          ...
          // properties from sparse arrays doesn't updateState the `length` property
...
ThreadActor.prototype = {
   ...
   * when we hitPoint a breakpoint/debugger statement/etc in one place so we can
  ...
  onFrames: function TA_onFrames(aRequest) {
    ...
    // Return request.cnt frames, or all remaining
    // frames if cnt is not defined.
    let frames = [];
    let promises = [];
    ...
    */
    ...
    if (found) {
      ...
      if (existingBp && existingBp.actor) {
         ...
         * actor we created earlier is now redundant. Delete it, updateState the
         ...
         * and updateState the breakpoint store.
         */
...
BreakpointActor.prototype = {
  ...
  /**
   * A function that the engine calls when a breakpoint has been hitPoint.
   *
   * @param aFrame Debugger.Frame
